			  ...
			  GtkAllocation *allocation)
{
  ...
  if (bin->child && GTK_WIDGET_VISIBLE (bin->child))
	...
	ratio = aspect_frame->ratio;
      ...
      x = GTK_CONTAINER (frame)->border_width+2; 
	   //+GTK_WIDGET (frame)->style->klass->xthickness);
      width = allocation->width - x * 2;
      
      y = (GTK_CONTAINER (frame)->border_width +frame->label_height) +5; //, GTK_WIDGET (frame)->style->klass->ythickness));
      height = (allocation->height - y  -
		GTK_CONTAINER (frame)->border_width);// - GTK_WIDGET (frame)->style->klass->ythickness);
      ...
      else
	{
      ...
      child_allocation.x = aspect_frame->xalign * (width - child_allocation.width) +  x;
      child_allocation.y = aspect_frame->yalign * (height - child_allocation.height) + y;
