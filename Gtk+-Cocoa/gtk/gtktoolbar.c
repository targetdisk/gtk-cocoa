			   ...
			   GtkAllocation *allocation)
{
  ...
  for (children = toolbar->children; children; children = children->next)
	  ...
	  if (toolbar->orientation == GTK_ORIENTATION_HORIZONTAL)
	    {
	      ...
	      child_space->alloc_y =  (allocation->height - toolbar->button_maxh) / 2;
	      alloc.x += toolbar->space_size;
	    }
	  else
	    {
	      child_space->alloc_x = (allocation->width - toolbar->button_maxw) / 2;
	...
	case GTK_TOOLBAR_CHILD_TOGGLEBUTTON:
	  ...
	  if (toolbar->orientation == GTK_ORIENTATION_HORIZONTAL)
	    alloc.y = (allocation->height - toolbar->button_maxh) / 2;
	  else
	    alloc.x = (allocation->width - toolbar->button_maxw) / 2;
	...
	case GTK_TOOLBAR_CHILD_WIDGET:
	  ...
	  if (toolbar->orientation == GTK_ORIENTATION_HORIZONTAL)
	    alloc.y = (allocation->height - child_requisition.height) / 2;
	  else
	    alloc.x = (allocation->width - child_requisition.width) / 2;
