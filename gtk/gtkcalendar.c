  ...
  if (calendar->display_flags & GTK_CALENDAR_SHOW_HEADING)
    {
      ...
      for (i = 0; i < 12; i++)
	{
	  private_data->max_month_width = MAX (private_data->max_month_width,8);
					       //gdk_string_measure (HEADER_FONT (widget), default_monthname[i]) + 8);
	}
      private_data->max_year_width = 0;
      for (i=0; i<10; i++)
	{
	  sprintf (buffer, "%d%d%d%d", i,i,i,i);
	  private_data->max_year_width = MAX (private_data->max_year_width,8);
					      // gdk_string_measure (HEADER_FONT (widget), buffer) + 8);
	}
    } 
  else 
    {
  ...
  for (i = 0; i < 9; i++)
    {
      ...
      private_data->min_day_width = MAX (private_data->max_day_char_width,8);
				     //gdk_string_measure (DAY_FONT (widget), buffer));
    }
...
*/
  ...
  if (calendar->display_flags & GTK_CALENDAR_SHOW_WEEK_NUMBERS)
    for (i = 0; i < 9; i++)
      {
	sprintf (buffer, "%d%d", i, i);
	private_data->max_week_char_width = MAX (private_data->max_week_char_width,8);
  ...
  if (calendar->display_flags & GTK_CALENDAR_SHOW_HEADING)
    {
      private_data->header_h = 16; //(HEADER_FONT (widget)->ascent + HEADER_FONT (widget)->descent + CALENDAR_YSEP * 2);
    }
  else
    {
			    ...
			    GtkAllocation *allocation)
{
  ...
  if (calendar->display_flags & GTK_CALENDAR_SHOW_WEEK_NUMBERS)
    {
      ...
      private_data->week_width = ((allocation->width - ( INNER_BORDER) * 2
			       - (CALENDAR_MARGIN * 2) - (DAY_XSEP * 7) - CALENDAR_XSEP * 2 )
  ...
  if (GTK_WIDGET_REALIZED (widget))
	...
	gdk_window_move_resize (private_data->day_name_win,
			INNER_BORDER,
				private_data->header_h + ( INNER_BORDER),
