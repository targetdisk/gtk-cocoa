...
gint  gtk_quit_invoke_function	 (GtkQuitFunction    *quitf);
void  gtk_quit_destroy		 (GtkQuitFunction    *quitf);
...
GSList *main_loops = NULL;      /* stack of currently executing main loops */

static GSList *grabs = NULL;		   /* A stack of unique grabs. The grabbing
					    *  widget is the first one on the list.
					    */
GList *init_functions = NULL;	   /* A list of init functions.
					    */
GList *quit_functions = NULL;	   /* A list of quit functions.
					    */
  ...
  if (marshal)
    {
      ...
      return gdk_idle_add_full (priority,
			      gtk_invoke_idle_timeout,
			      closure,
			      gtk_destroy_closure);
    }
  else
    return gdk_idle_add_full (priority, function, data, destroy);
}
...
gtk_idle_add (GtkFunction function,
	      gpointer	  data)
{
  return gdk_idle_add_full (GTK_PRIORITY_DEFAULT, function, data, NULL);
 }
...
gtk_idle_add_priority	(gint		    priority,
			 ...
			 gpointer	    data)
{
 return gdk_idle_add_full (priority, function, data, NULL);
}
...
{
  if (!gdk_idle_remove_by_data (data))
