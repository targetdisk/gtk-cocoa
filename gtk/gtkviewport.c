			   ...
			   GtkRequisition *requisition)
{
  ...
  requisition->width = GTK_CONTAINER (widget)->border_width *2 +5;
			//GTK_WIDGET (widget)->style->klass->xthickness) * 2  +  5;

  requisition->height = GTK_CONTAINER (widget)->border_width * 2 + 5;
			    ...
			    GtkAllocation *allocation)
{
  ...
  if (viewport->shadow_type != GTK_SHADOW_NONE)
    {
      child_allocation.x = 0;// GTK_WIDGET (viewport)->style->klass->xthickness;
      child_allocation.y = 0;//GTK_WIDGET (viewport)->style->klass->ythickness;
    }
