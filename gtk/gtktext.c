...
void init_properties      (GtkText *text);
		       ...
		       GtkRequisition *requisition)
{
  ...
  char_height = MIN_TEXT_HEIGHT_LINES;//(widget->style->font->ascent + widget->style->font->descent);
  
  char_width = MIN_TEXT_WIDTH_LINES ;
  //(gdk_text_width (widget->style->font,"ABCDEFGHIJKLMNOPQRSTUVWXYZ", 26)/ 26);
...
void
gtk_text_insert_gtk (GtkText    *text,
		 GdkFont    *font,
		 GdkColor   *fore,
		 GdkColor   *back,
		 const char *chars,
...
//      numwcs = gdk_mbstowcs (text->text.wc + text->gap_position, chars_nt, length);
      if (chars_nt != chars)
	g_free(chars_nt);
      if (numwcs < 0)
	numwcs = 0;
