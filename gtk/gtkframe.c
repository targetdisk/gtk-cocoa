...
// +			GTK_WIDGET (widget)->style->klass->xthickness) * 2;
  ...
  tmp_height = frame->label_height;
// - GTK_WIDGET (widget)->style->klass->ythickness;
  tmp_height = MAX (tmp_height, 0);

  requisition->height = tmp_height + (GTK_CONTAINER (widget)->border_width)*2 +8;
			 ...
			 GtkAllocation *allocation)
{
  ...
  if (bin->child && GTK_WIDGET_VISIBLE (bin->child))
    {
      child_allocation.x = (GTK_CONTAINER (frame)->border_width) ; // + GTK_WIDGET (frame)->style->klass->xthickness);
      child_allocation.width = MAX(1, (gint)allocation->width - child_allocation.x * 2 - 4);

      child_allocation.y = (GTK_CONTAINER (frame)->border_width)  + 4;
      child_allocation.height = MAX (1, (gint)allocation->height - child_allocation.y*2  - frame->label_height);
      child_allocation.y += frame->label_height+2;
